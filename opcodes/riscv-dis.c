...
static const char *const *riscv_mpr_names;
...
set_default_riscv_dis_options(void)
{
	...
	riscv_mpr_names = riscv_mat_names_numeric;
	no_aliases = 0;
}
...
print_insn_args(const char *d, insn_t l, bfd_vma pc, disassemble_info *info)
{
	...
	for (; *d != '\0'; d++)
	{
		...
		case 'o':
			...
			/* Fall through.  */
		case 'j':
			if (((l & MASK_ADDI) == MATCH_ADDI && rs1 != 0) || (l & MASK_JALR) == MATCH_JALR)
				maybe_print_address(pd, rs1, EXTRACT_ITYPE_IMM(l));
...
#define DECLARE_CSR(name, num, class, define_version, abort_version)                                                                                                                                      \
	if (riscv_csr_hash[num] == NULL && ((define_version == PRIV_SPEC_CLASS_NONE && abort_version == PRIV_SPEC_CLASS_NONE) || (default_priv_spec >= define_version && default_priv_spec < abort_version))) \
...
#undef DECLARE_CSR
		...
		case 'M':
			...
			{
			case 'z':
				print(info->stream, "%lu", (l >> 12) & 0x7);
				break;
			case 'd':
				print(info->stream, "%s", riscv_mpr_names[rd]);
				break;
			case 's':
				switch (*++d)
				{
				case '1':
					print(info->stream, "%s", riscv_mpr_names[rs1]);
					break;
				case '2':
					print(info->stream, "%s", riscv_mpr_names[rs2]);
					break;
				default:
					fprintf(stderr, "Ms%c", *d);
					break;
				}
				break;
			default:
