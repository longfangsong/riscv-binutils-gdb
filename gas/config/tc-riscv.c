...
init_ext_version_hash(const struct riscv_ext_version *table)
{
  ...
  while (table[i].name)
  {
    ...
    i++;
    while (table[i].name && strcmp(table[i].name, name) == 0)
      i++;
  }
                              ...
                              int *minor_version)
{
  ...
  while (ext && ext->name && strcmp(ext->name, name) == 0)
  {
...
#define RELAX_BRANCH_ENCODE(uncond, rvc, length) \
  ((relax_substateT)(0xc0000000 | ((uncond) ? 1 : 0) | ((rvc) ? 2 : 0) | ((length) << 2)))
...
#define IS_SEXT_32BIT_NUM(x) \
  (((x) & ~(offsetT)0x7fffffff) == 0 || (((x) & ~(offsetT)0x7fffffff) == ~(offsetT)0x7fffffff))

/* Is the given value a zero-extended 32-bit value?  Or a negated one?  */
#define IS_ZEXT_32BIT_NUM(x) \
  (((x) & ~(offsetT)0xffffffff) == 0 || (((x) & ~(offsetT)0xffffffff) == ~(offsetT)0xffffffff))
...
relaxed_branch_length(fragS *fragp, asection *sec, int update)
{
  ...
  if (fragp->fr_symbol != NULL && S_IS_DEFINED(fragp->fr_symbol) && !S_IS_WEAK(fragp->fr_symbol) && sec == S_GET_SEGMENT(fragp->fr_symbol))
  {
                    ...
                    enum riscv_priv_spec_class abort_version)
{
  ...
  while (need_enrty && entry != NULL)
  {
    if (entry->csr_class == class
        && entry->address == address && entry->define_version == define_version && entry->abort_version == abort_version)
      need_enrty = FALSE;
    pre_entry = entry;
    entry = entry->next;
  }
                  ...
                  struct riscv_csr_extra *entry)
{
  ...
  while (entry != NULL)
  {
    if (!need_check_version || (default_priv_spec >= entry->define_version && default_priv_spec < entry->abort_version))
    {
...
#define USE_BITS(mask, shift) (used_bits |= ((insn_t)(mask) << (shift)))
  while (*p)
    ...
    case 'C': /* RVC */
      ...
      case 'z':
        break; /* RS2S, contrained to be x0 */
      case 'K':
            ...
            bfd_reloc_code_real_type reloc_type)
{
  ...
  if (reloc_type == BFD_RELOC_RISCV_HI20 || reloc_type == BFD_RELOC_RISCV_PCREL_HI20 || reloc_type == BFD_RELOC_RISCV_TPREL_HI20 || reloc_type == BFD_RELOC_RISCV_TPREL_ADD)
  {
    frag_wane(frag_now);
    frag_new(0);
  }
...
normalize_constant_expr(expressionS *ex)
{
  if (xlen > 32)
    return;
  if ((ex->X_op == O_constant || ex->X_op == O_symbol) && IS_ZEXT_32BIT_NUM(ex->X_add_number))
    ex->X_add_number = (((ex->X_add_number & 0xffffffff) ^ 0x80000000) - 0x80000000);
}
...
{
  if (((insn ^ MATCH_CSRRW) & MASK_CSRRW) == 0 || ((insn ^ MATCH_CSRRWI) & MASK_CSRRWI) == 0)
    return INSN_CSRRW;
  else if (((insn ^ MATCH_CSRRS) & MASK_CSRRS) == 0 || ((insn ^ MATCH_CSRRSI) & MASK_CSRRSI) == 0)
    return INSN_CSRRS;
  else if (((insn ^ MATCH_CSRRC) & MASK_CSRRC) == 0 || ((insn ^ MATCH_CSRRCI) & MASK_CSRRCI) == 0)
    return INSN_CSRRC;
  else
    return INSN_NOT_CSR;
}
...
riscv_csr_read_only_check(insn_t insn)
{
  ...
  if (readonly && (((csr_insn == INSN_CSRRS || csr_insn == INSN_CSRRC) && rs1 != 0) || csr_insn == INSN_CSRRW))
    return FALSE;
...
{
  return (((insn ^ MATCH_SRET) & MASK_SRET) == 0 || ((insn ^ MATCH_MRET) & MASK_MRET) == 0 || ((insn ^ MATCH_SFENCE_VMA) & MASK_SFENCE_VMA) == 0 || ((insn ^ MATCH_WFI) & MASK_WFI) == 0
    ...
    *imm_reloc = BFD_RELOC_UNUSED;
    ...
    for (args = insn->args;; ++args)
    {
      ...
      case '\0': /* End of args.  */
        if (insn->pinfo != INSN_MACRO)
        {
          ...
          /* Check if we write a read-only CSR by the CSR
		     instruction.  */
          if (insn_with_csr && riscv_opts.csr_check && !riscv_csr_read_only_check(ip->insn_opcode))
          {
      ...
      case 'C': /* RVC */
        ...
        {
        case 's': /* RS1 x8-x15 */
          if (!reg_lookup(&s, RCLASS_GPR, &regno) || !(regno >= 8 && regno <= 15))
            break;
          INSERT_OPERAND(CRS1S, *ip, regno % 8);
          continue;
        case 'w': /* RS1 x8-x15, constrained to equal RD x8-x15.  */
          if (!reg_lookup(&s, RCLASS_GPR, &regno) || EXTRACT_OPERAND(CRS1S, ip->insn_opcode) + 8 != regno)
            break;
          continue;
        case 't': /* RS2 x8-x15 */
          if (!reg_lookup(&s, RCLASS_GPR, &regno) || !(regno >= 8 && regno <= 15))
        ...
        case 'x': /* RS2 x8-x15, constrained to equal RD x8-x15.  */
          if (!reg_lookup(&s, RCLASS_GPR, &regno) || EXTRACT_OPERAND(CRS2S, ip->insn_opcode) + 8 != regno)
            break;
          continue;
        case 'U': /* RS1, constrained to equal RD.  */
          if (!reg_lookup(&s, RCLASS_GPR, &regno) || EXTRACT_OPERAND(RD, ip->insn_opcode) != regno)
            break;
          continue;
        case 'V': /* RS2 */
        ...
        case 'z': /* RS2, contrained to equal x0.  */
          ...
          continue;
        case '>':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number <= 0 || imm_expr->X_add_number >= 64)
            break;
        ...
        rvc_imm_done:
          ...
          continue;
        case '<':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number <= 0 || imm_expr->X_add_number >= 32 || !VALID_RVC_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |= ENCODE_RVC_IMM(imm_expr->X_add_number);
          goto rvc_imm_done;
        case '8':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number < 0 || imm_expr->X_add_number >= 256 || !VALID_RVC_UIMM8((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |= ENCODE_RVC_UIMM8(imm_expr->X_add_number);
          goto rvc_imm_done;
        case 'i':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number == 0 || !VALID_RVC_SIMM3((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |= ENCODE_RVC_SIMM3(imm_expr->X_add_number);
          goto rvc_imm_done;
        case 'j':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number == 0 || !VALID_RVC_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |= ENCODE_RVC_IMM(imm_expr->X_add_number);
          goto rvc_imm_done;
        case 'k':
          if (riscv_handle_implicit_zero_offset(imm_expr, s))
            continue;
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || !VALID_RVC_LW_IMM((valueT)imm_expr->X_add_number))
            break;
        ...
        case 'l':
          if (riscv_handle_implicit_zero_offset(imm_expr, s))
            continue;
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || !VALID_RVC_LD_IMM((valueT)imm_expr->X_add_number))
            break;
        ...
        case 'm':
          if (riscv_handle_implicit_zero_offset(imm_expr, s))
            continue;
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || !VALID_RVC_LWSP_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |=
        ...
        case 'n':
          if (riscv_handle_implicit_zero_offset(imm_expr, s))
            continue;
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || !VALID_RVC_LDSP_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |=
              ENCODE_RVC_LDSP_IMM(imm_expr->X_add_number);
          goto rvc_imm_done;
        case 'o':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant
          ...
          goto rvc_imm_done;
        case 'K':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number == 0 || !VALID_RVC_ADDI4SPN_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |=
              ENCODE_RVC_ADDI4SPN_IMM(imm_expr->X_add_number);
          goto rvc_imm_done;
        case 'L':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number == 0 || !VALID_RVC_ADDI16SP_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |=
        ...
        case 'M':
          if (riscv_handle_implicit_zero_offset(imm_expr, s))
            continue;
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || !VALID_RVC_SWSP_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |=
        ...
        case 'N':
          if (riscv_handle_implicit_zero_offset(imm_expr, s))
            continue;
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || !VALID_RVC_SDSP_IMM((valueT)imm_expr->X_add_number))
            break;
          ip->insn_opcode |=
        ...
        case 'u':
          ...
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p))
            break;
        rvc_lui:
          if (imm_expr->X_op != O_constant || imm_expr->X_add_number <= 0 || imm_expr->X_add_number >= RISCV_BIGIMM_REACH || (imm_expr->X_add_number >= RISCV_RVC_IMM_REACH / 2 && (imm_expr->X_add_number < RISCV_BIGIMM_REACH - RISCV_RVC_IMM_REACH / 2)))
            break;
          ip->insn_opcode |= ENCODE_RVC_IMM(imm_expr->X_add_number);
          goto rvc_imm_done;
        case 'v':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || (imm_expr->X_add_number & (RISCV_IMM_REACH - 1)) || ((int32_t)imm_expr->X_add_number != imm_expr->X_add_number))
            break;
        ...
        case 'S': /* Floating-point RS1 x8-x15.  */
          if (!reg_lookup(&s, RCLASS_FPR, &regno) || !(regno >= 8 && regno <= 15))
            break;
          INSERT_OPERAND(CRS1S, *ip, regno % 8);
          continue;
        ...
        case 'F':
          ...
          case '6':
            ...
            continue;
          case '4':
            if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number < 0 || imm_expr->X_add_number >= 16)
            {
            ...
            continue;
          case '3':
            if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number < 0 || imm_expr->X_add_number >= 8)
            {
            ...
            continue;
          case '2':
            if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number < 0 || imm_expr->X_add_number >= 4)
            {
      ...
      case 'M':
        ...
        case 'z':
          ...
          if (*s == ' ')
              ++s;
          INSERT_OPERAND(MAT_SIZE, *ip, matrix_size);
          break;
        case 'd':
          if (reg_lookup(&s, RCLASS_MPR, &regno))
          {
            c = *args;
            if (*s == ' ')
              ++s;
      ...
      alu_op:
        ...
        {
          normalize_constant_expr(imm_expr);
          if (imm_expr->X_op != O_constant || (*args == '0' && imm_expr->X_add_number != 0) || (*args == '1') || imm_expr->X_add_number >= (signed)RISCV_IMM_REACH / 2 || imm_expr->X_add_number < -(signed)RISCV_IMM_REACH / 2)
            break;
        }
      ...
      case 'u': /* Upper 20 bits.  */
        ...
        if (!my_getSmallExpression(imm_expr, imm_reloc, s, p))
        {
          ...
          if (imm_expr->X_add_number < 0 || imm_expr->X_add_number >= (signed)RISCV_BIGIMM_REACH)
      ...
      case 'O':
        ...
        {
        case '4':
          if (my_getOpcodeExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number < 0 || imm_expr->X_add_number >= 128 || (imm_expr->X_add_number & 0x3) != 3)
          {
          ...
          continue;
        case '2':
          if (my_getOpcodeExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number < 0 || imm_expr->X_add_number >= 3)
          {
            as_bad(_("bad value for opcode field, "
                     "value must be 0...2"));
            break;
      ...
      case 'F':
        ...
        {
        case '7':
          if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number < 0 || imm_expr->X_add_number >= 128)
          {
      ...
      case 'z':
        if (my_getSmallExpression(imm_expr, imm_reloc, s, p) || imm_expr->X_op != O_constant || imm_expr->X_add_number != 0)
          break;
        s = expr_end;
        imm_expr->X_op = O_absent;
        continue;
...
void md_apply_fix(fixS *fixP, valueT *valP, segT seg ATTRIBUTE_UNUSED)
{
  ...
  case BFD_RELOC_RISCV_LO12_I:
  case BFD_RELOC_RISCV_LO12_S:
    bfd_putl32(riscv_apply_const_reloc(fixP->fx_r_type, *valP) | bfd_getl32(buf), buf);
  ...
  case BFD_RELOC_32:
    ...
    if (fixP->fx_addsy && fixP->fx_subsy && (sub_segment = S_GET_SEGMENT(fixP->fx_subsy)) && strcmp(sub_segment->name, ".eh_frame") == 0 && S_GET_VALUE(fixP->fx_subsy) == fixP->fx_frag->fr_address + fixP->fx_where)
    {
...
void riscv_handle_align(fragS *fragP)
{
  ...
  case rs_align_code:
    ...
    if (!riscv_opts.relax)
    {
      bfd_signed_vma bytes = (fragP->fr_next->fr_address - fragP->fr_address - fragP->fr_fix);
...
tc_gen_reloc(asection *section ATTRIBUTE_UNUSED, fixS *fixp)
{
  ...
  *reloc->sym_ptr_ptr = symbol_get_bfdsym(fixp->fx_addsy);
  ...
  if (reloc->howto == NULL)
  {
    if ((fixp->fx_r_type == BFD_RELOC_16 || fixp->fx_r_type == BFD_RELOC_8) && fixp->fx_addsy != NULL && fixp->fx_subsy != NULL)
    {
...
md_convert_frag_branch(fragS *fragp)
{
  ...
  gas_assert(buf == (bfd_byte *)fragp->fr_literal + fragp->fr_fix + fragp->fr_var);
...
riscv_write_out_attrs(void)
{
  ...
  /* For the file without any instruction, we don't set the default_priv_spec
     ...
     it seems strange.  */
  if (!start_assemble && !riscv_set_default_priv_spec(NULL))
    return;
...
int riscv_convert_symbolic_attribute(const char *name)
{
  static const struct
  {
    const char *name;
